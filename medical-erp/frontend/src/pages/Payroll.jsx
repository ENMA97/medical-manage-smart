      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="divide-y divide-gray-50">
            {payrolls.map((p) => (
              <div key={p.id} className="px-4 py-3 hover:bg-gray-50 flex items-center justify-between">
                <div>
                  <p className="font-medium text-gray-800">{months[(p.month || 1) - 1]} {p.year}</p>
                  <p className="text-xs text-gray-500 mt-0.5">
                    {p.total_amount ? `${Number(p.total_amount).toLocaleString()} ريال` : '—'} — {p.items_count || 0} موظف
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {p.status === 'draft' && (
                    <button onClick={() => handleApprove(p.id)} className="text-xs text-green-600 hover:text-green-800 font-medium">اعتماد</button>
                  )}
                  <button onClick={() => handleExport(p.id)} className="text-xs text-blue-600 hover:text-blue-800 font-medium">تصدير</button>
                </div>
              </div>
            ))}
          </div>
