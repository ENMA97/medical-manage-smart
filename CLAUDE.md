...
```
┌────────────┐   ┌──────────────────────────────────────┐   ┌──────────┐
│ إنشاء     │──▶│         المدير الإداري               │──▶│ معتمد    │
│ القرار    │   │                                      │   │          │
└────────────┘   │  ┌─────────────────────────────┐    │   └──────────┘
                 │  │ خيار 1: اعتماد مباشر ✓     │    │
                 │  │ خيار 2: تحويل للمدير العام │    │
                 │  └─────────────────────────────┘    │
                 └──────────────────────────────────────┘
                                   │
                                   │ (حسب الحاجة)
...
| **المدير الإداري** | القرار | اعتماد | اعتماد مباشر أو تحويل للمدير العام |
| **المدير الطبي** | القرار | اعتماد | اعتماد مباشر أو تحويل للمدير العام |
| **المدير العام** | القرار | اعتماد | اعتماد اختياري (حسب الحاجة والتحويل) |

#### خيارات المدير الإداري/الطبي في القرار:
| الخيار | الوصف |
|--------|-------|
| **اعتماد مباشر** | الموافقة النهائية على القرار |
| **تحويل للمدير العام** | إحالة القرار للمدير العام للمرونة التشغيلية |
| **رفض** | رفض القرار مع ذكر السبب |
...
Leave Decision:  draft → pending_admin_manager/pending_medical_director → approved (أو → pending_general_manager → approved)
```
